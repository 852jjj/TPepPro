...
cmaproot = './data/xxx_cmap/'
embed_data = np.load("./data/dictionary/xxx_embeddings.npz")


def default_loader(cpath, pid):
    cmap_data = np.load(cpath)
    nodenum = len(str(cmap_data['seq']))
    #print(cmap_data['seq'])#add
    #print(nodenum)#add
    cmap = cmap_data['contact']
    #print(cmap.shape)#add
...
class MyDataset(Dataset):
    ...
    '''def __init__(self,type,record,transform=None,target_transform=None, loader=default_loader):
        
        super(MyDataset,self).__init__()
        pns=[]
        with open('./data/'+datasetname+'/actions/'+record+'/'+type+'_cmap.actions.tsv', 'r') as fh:
            for line in fh: 
                line = line.strip('\n')
                line = line.rstrip('\n')
                words = re.split('  |\t',line)
